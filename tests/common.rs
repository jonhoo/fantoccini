...
use fantoccini::{error, Client, Tls};
...
pub async fn select_client_type(s: &str) -> Result<Client, error::NewSessionError> {
    match s {
        "firefox" => {
            ...
            let opts = serde_json::json!({ "args": ["--headless"] });
            caps.insert("moz:firefoxOptions".to_string(), opts.clone());
            Client::with_capabilities("http://localhost:4444", caps, Tls::OpenSSL).await
        }
        "chrome" => {
            let mut caps = serde_json::map::Map::new();
            let opts = serde_json::json!({
            ...
            caps.insert("goog:chromeOptions".to_string(), opts.clone());
            Client::with_capabilities("http://localhost:9515", caps, Tls::OpenSSL).await
        }
        browser => unimplemented!("unsupported browser backend {}", browser),
    }
}
